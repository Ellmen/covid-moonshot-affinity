...
def fingerprint_cluster(data_path, out_path):
    ...
    for i in range(10):
        ...
        plt.savefig(f'outputs/fingerprint_clusters_{cutoff}.pdf')
    plt.clf()
    plt.plot(cutoffs, num_clusters)
    plt.ylabel('Number of clusters')
    plt.xlabel('Cutoff value')
    plt.savefig('outputs/fingerprint_cluster_cutoffs.pdf')
    con.close()

fingerprint_cluster(snakemake.input[0], snakemake.output[0])
